...
export type SpLogicalState = "waiting-for-tune" | "calling-cq" | "busy-with-other-station" | "need-operator-call" | "need-operator-number" | "need-operator-call-and-number" | "ready-to-acknowledge" | "closing" | "cooldown" | "completed" | "failed" | "aborted";
export type SpPhysicalState = "listening" | "copying" | "preparing" | "sending";
export type SpQsoPhase = "idle" | "listening-cq" | "station-busy" | "calling-station" | "awaiting-reply" | "receiving-exchange" | "sending-our-exchange" | "station-requesting-call" | "station-requesting-number" | "station-requesting-again" | "awaiting-tu" | "completed" | "failed" | "aborted";
export type SpIncident = "busy" | "ignore-first-call" | "request-agn" | "request-call" | "request-number" | "repeat-exchange" | "partial-operator-call" | "b4" | "nil" | "qrl" | "qsy" | "wrong-number-correction";
export type SpStationMessage = "busy" | "exchange" | "agn" | "call" | "number" | "final" | "tu" | "b4" | "nil" | "qrl" | "qsy";
...
export function checkSpQso(entry: { call: string; rst: string; exchange: string }, scenario: SpQsoScenario, duplicate = false): QsoCheck { if (duplicate) return "DUP"; const call = compareCallsign(entry.call, scenario.stationCall); if (call !== "exact") return call === "almost" ? "CALL" : "NIL"; if (normalize(entry.rst) !== scenario.rst) return "RST"; return normalize(entry.exchange) === scenario.exchange ? "OK" : "NR"; }
const has = (state: SpQsoState, item: SpIncident): boolean => state.scenario?.incidents.includes(item) === true && !state.usedIncidents.includes(item);
const consume = (state: SpQsoState, item: SpIncident): SpQsoState => ({ ...state, usedIncidents: state.usedIncidents.includes(item) ? state.usedIncidents : [...state.usedIncidents, item] });
const station = (state: SpQsoState, text: string, message: SpStationMessage, phase: SpQsoPhase, logical: SpLogicalState, delayed = false): SpQsoTransition => ({ state: { ...state, phase, logical: delayed ? state.returnLogical ?? logical : logical, physical: "sending", lastStationText: text }, effects: [{ type: "play-station", text, message, ...(delayed ? { delayMs: state.scenario?.responseDelayMs } : {}) }] });
const withTimeoutCancel = (transition: SpQsoTransition): SpQsoTransition => ({ ...transition, effects: [{ type: "cancel-reply-timeout" }, ...transition.effects] });
const startReplyTimeout = (state: SpQsoState): SpQsoEffect => ({ type: "start-reply-timeout", delayMs: state.scenario!.profile.replyTimeoutMs });
const finalAcknowledgement = (state: SpQsoState, scenario: SpQsoScenario): string => scenario.profile.acknowledgement === "r-number-tu" ? `R ${state.serial} TU` : scenario.profile.acknowledgement === "rr-tu" ? "RR TU" : "TU";
const stationExchange = (state: SpQsoState, scenario: SpQsoScenario): string => {
  if (!has(state, "wrong-number-correction")) return `${scenario.operatorCall} 5NN ${scenario.exchange}`;
  const wrong = String(Number(scenario.exchange) + 1).padStart(scenario.exchange.length, "0");
  return `${scenario.operatorCall} 5NN ${wrong} EEEEE ${scenario.exchange}`;
};
const endWithoutQso = (state: SpQsoState, status: "FAILED" | "WORKED" | "QSY" = "FAILED"): SpQsoTransition => ({ state: { ...state, phase: "failed", logical: "failed", physical: "listening" }, effects: [{ type: "cancel-reply-timeout" }, { type: "status", label: status === "WORKED" ? "QSO B4" : status === "QSY" ? "ESTAÇÃO QSY" : "QSO ENCERRADO" }, { type: "update-spot-status", status }, { type: "restart-cq" }] });
const op = (state: SpQsoState, text: string, pending: NonNullable<SpQsoState["pendingOperator"]>, label: string, phase: SpQsoPhase = "calling-station", returnPhase?: SpQsoPhase, returnLogical?: SpLogicalState): SpQsoTransition => ({ state: { ...state, phase, logical: phase === "sending-our-exchange" ? "need-operator-number" : "need-operator-call", physical: "sending", pendingOperator: pending, returnPhase, returnLogical, lastOperatorText: text }, effects: [{ type: "cancel-reply-timeout" }, { type: "play-operator", text }, { type: "status", label }] });
const partialCallCopy = (call: string): string => {
  if (call.length >= 4) return `${call.slice(0, -2)}?${call.slice(-1)}`;
...
const respondToOperatorCall = (state: SpQsoState, scenario: SpQsoScenario): SpQsoTransition => {
  if (has(state, "b4")) return station(consume(state, "b4"), "QSO B4", "b4", "awaiting-tu", "closing", true);
  if (has(state, "qrl")) return station(consume(state, "qrl"), "QRL", "qrl", "awaiting-tu", "closing", true);
  if (has(state, "qsy")) return station(consume(state, "qsy"), "QSY", "qsy", "awaiting-tu", "closing", true);
  if (has(state, "busy")) return station(consume(state, "busy"), "W1AW 5NN 047 TU", "busy", "station-busy", "busy-with-other-station");
  if (has(state, "ignore-first-call")) return { state: { ...consume(state, "ignore-first-call"), phase: "listening-cq", logical: "calling-cq", physical: "listening" }, effects: [{ type: "status", label: "SEM RESPOSTA" }, { type: "restart-cq" }] };
  const policy = scenario.callCopyPolicy ?? { model: "legacy" as const, skill: scenario.skill, acceptAlmost: false, rejectExact: false };
  const transmittedOperatorCall = state.transmittedOperatorCall ?? state.lastOperatorText ?? scenario.operatorCall;
  const partialIncident = has(state, "partial-operator-call");
...
export function reduceSpQso(state: SpQsoState, event: SpQsoEvent): SpQsoTransition {
  ...
  if (event.type === "operator-transmitted") {
    ...
    const intent = parseContestTransmission(event.text, { operatorCall: s.operatorCall, stationCall: s.stationCall, operatorSerial: state.serial, stationExchange: s.exchange, phase });
    if (intent.kind === "call-station") return reduceSpQso(state, { type: "operator-call", text: intent.copiedCall ?? s.operatorCall });
    if (intent.kind === "send-exchange" || intent.kind === "send-call-and-exchange") return reduceSpQso(state, { type: "operator-exchange", call: intent.call ?? s.stationCall, rst: intent.rst ?? "599", exchange: intent.serial ?? "", text: event.text });
    if (intent.kind === "request-again") return reduceSpQso(state, { type: "operator-agn" });
    if (intent.kind === "request-call") return reduceSpQso(state, { type: "operator-call-request" });
    if (intent.kind === "request-number") return reduceSpQso(state, { type: "operator-number-request" });
  ...
  if (event.type === "operator-tu") return { state, effects: [{ type: "play-operator", text: "TU" }] };
  if (event.type === "operator-agn" && state.lastStationText) return withTimeoutCancel(op(state, "AGN", "agn-request", "AGN", "calling-station", state.phase, state.logical));
  if (event.type === "operator-number-request") return withTimeoutCancel(op(state, "NR?", "number-request", "NR?", "calling-station", state.phase, state.logical));
  if (event.type === "operator-call-request") return withTimeoutCancel(op(state, "CALL?", "call-request", "CALL?", "calling-station", state.phase, state.logical));
  if (event.type === "station-finished" && (event.message === "b4" || event.message === "qrl" || event.message === "qsy") && state.phase === "awaiting-tu") return endWithoutQso(state, event.message === "b4" ? "WORKED" : event.message === "qsy" ? "QSY" : "FAILED");
